...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form method="POST" action="{{ route('admin.villages.update', $village) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('name', $village->name) }}" required>
                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Kepala Desa</label>
                    <input type="text" name="head_name" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('head_name', $village->head_name) }}">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Kantor Desa</label>
                    <textarea name="address" rows="3" class="w-full border-gray-300 rounded-md shadow-sm">{{ old('address', $village->address) }}</textarea>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Telepon</label>
                        <input type="text" name="phone" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('phone', $village->phone) }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" name="email" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('email', $village->email) }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    @if($village->logo)
                        <img src="{{ Storage::url($village->logo) }}" class="h-20 w-auto object-cover rounded mb-2">
                    @else
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300" {{ $village->is_active ? 'checked' : '' }}>
